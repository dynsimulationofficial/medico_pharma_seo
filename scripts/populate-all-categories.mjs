...
// Seed initial products data for categories
const initialProductsMap = {
  "pharmaceutical-tablets": [
    ...
    { id: "23144505112", name: "25 Mg Deprid Imipramine Hydrochloride Tablets", price: "Price on Request", remoteImage: "https://5.imimg.com/data5/ANDROID/Default/2024/1/381458996/YE/HR/NI/15001533/product-jpeg-500x500.jpg" },
    { id: "23144517488", name: "75 Mg Deprid Imipramine Hydrochloride Tablets", price: "Price on Request", remoteImage: "https://5.imimg.com/data5/ANDROID/Default/2024/1/381458839/UI/VQ/OG/15001533/product-jpeg-500x500.jpg" },
    { id: "2852106726412", name: "Sildigra Gold Sildenafil Citrate Tablet", price: "Price on Request", remoteImage: "https://5.imimg.com/data5/SELLER/Default/2025/7/530766692/XV/CP/RG/15001533/img-20240126-164803-500x500.jpg" }
  ],
...
// Populate full category database
const fullCategories = categoriesList.map((cat) => {
  const seedProds = initialProductsMap[cat.slug] || [
    {
      id: Math.random().toString().slice(2, 12),
      name: `${cat.name} Formulation`,
  ...
  const formattedProducts = seedProds.map((p) => {
  ...
  return {
    ...cat,
    description: cat.description || `Explore high-quality ${cat.name} range supplied by Medico Pharma.`,
...
fullCategories.forEach((c) => (totalProductsCount += c.products.length));
...
const masterDatabase = {
  ...
  totalCategories: fullCategories.length,
...
// Also update categories.json with exact product counts
const categoriesClean = fullCategories.map((c) => ({
  id: c.id,
  name: c.name,
  slug: c.slug,
...
fs.writeFileSync(
  ...
  JSON.stringify(categoriesClean, null, 2),
  "utf-8"
);

console.log(`Successfully populated ALL ${fullCategories.length} categories with total ${totalProductsCount} products!`);
