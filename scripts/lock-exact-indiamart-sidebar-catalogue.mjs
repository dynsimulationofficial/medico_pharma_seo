...
// Master Dictionary locked to IndiaMART screenshots & sidebars
const exactCatalogMap = {
...
db.categories.forEach((cat) => {
  if (exactCatalogMap[cat.slug]) {
    cat.products = exactCatalogMap[cat.slug].map((p) => {
  ...
  console.log(`[LOCKED] Category [${cat.name}] (${cat.slug}) -> EXACT ${cat.productCount} products`);
});
...
console.log(`\n==================================================`);
console.log(`LOCKED! All categories set to exact official IndiaMART counts.`);
